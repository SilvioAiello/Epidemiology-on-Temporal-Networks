...
def STRUCTURE_GEN_ANALYSIS(N,T, alfa, NET_REAL, P, q_param, isDIRECTED, isDAR,isSAMPLED,fig_count, directory_name):
    fig_count+=50
    if isDAR:
                                #DAR NETWORK GENERATION#
        #SAMPLED DATA IMPORT
...
#            chi = Evolutions.input_sampling(chiDAR1,N=N,isDAR = isDAR)
            ...
            chi = 0.4*(np.ones(shape = (N,N)) - np.diag([1 for i in range(N)]))
...
#            sigmaTGRG = sigmaTGRG[1:98]
    ...
    #FROM HERE, OPERATIONS ARE FUNTIONALIZED TO ALLOW USER TO SUSPEND THEM. IF IT'S CASE, LET OUTPUT = 1
    ...
    if isDAR:
        return temporal_networks_RTNN, alpha, chi, 1,communicabilities_RTNN,1,1,1,1, fig_count
...
def INFECTION_GEN_ANALYSIS(N,T, beta, NET_REAL, K, isDAR, infective_fraction,multiple_infections, temporal_networks_RTNN,communicabilities_RTNN, q_param, directory_name):
    ...
    for netw_realiz in range(NET_REAL):            
        ...
        for index_case in range(N):
            ...
            for iteration in range(K):
                infection_matrix[index_case].append(Propagation_SI.propagation_v2(temporal_networks_RTNN[netw_realiz], index_case, probabilities, multiple_infections = multiple_infections)[0])
