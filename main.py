...
#%%                         INPUTS BUILDING AND SAVE
    ...
    if isDAR:
        ...
        #alpha
        if isSAMPLED:
            fig_count = Evolutions.hist_plots(fig_count,alpha,N,isDAR,r"$\alpha$ values generated by empiric distribution",directory_name+"/histALPHA.pdf")
            fig_count = Evolutions.hist_plots(fig_count,chi,N,isDAR,r"$\chi$ values generated by empiric distribution",directory_name+"/histCHI.pdf")
        else:
            fig_count = Evolutions.hist_plots(fig_count,alpha,N,isDAR,r"\alpha$ values generated by beta distribution",directory_name+"/histALPHA.pdf")
            fig_count = Evolutions.hist_plots(fig_count,chi,N,isDAR,r"\chi$ values generated by empiric distribution",directory_name+"/histCHI.pdf")
        #Pickles
...
#            sigmaTGRG = sigmaTGRG[1:98]
        ...
        #alpha
        if isSAMPLED:
            fig_count = Evolutions.hist_plots(fig_count,phi0,N,isDAR,r"$\phi_0$ values generated by empiric distribution",directory_name+"/histPHI0.pdf")
            fig_count = Evolutions.hist_plots(fig_count,phi1,N,isDAR,r"$\phi_1$ values generated by empiric distribution",directory_name+"/histPHI1.pdf")
            fig_count = Evolutions.hist_plots(fig_count,sigma,N,isDAR,r"$\sigma$ values generated by empiric distribution",directory_name+"/histSIGMA.pdf")
        else:
            fig_count = Evolutions.hist_plots(fig_count,phi0,N,isDAR,r"$\phi_0$ values generated by analytic distribution",directory_name+"/histPHI0.pdf")
            fig_count = Evolutions.hist_plots(fig_count,phi1,N,isDAR,r"$\phi_1$ values generated by analytic distribution",directory_name+"/histPHI1.pdf")
            fig_count = Evolutions.hist_plots(fig_count,sigma,N,isDAR,r"$\sigma$ values generated by analytic distribution",directory_name+"/histSIGMA.pdf")
        #Pickles        
...
#%%                         OUTPUTS GENERATION
    ...
    #Following lists contain NET_REAL arrays; each of these arrays has T entries, with the average size at each time
    degree_evolution = []
    ...
    for k in range(1,NET_REAL+1):  #so first realization has index 1; for each k one tempnet and one k are overwritten
        ...
        for index_case in range(N):
            ...
            for iteration in range(K):
                ...
                epidemic_size_evolution.append([Propagation_SI.infected_counter(label[index_case][iteration][t],N) for t in range(T)])
...
#%% RESULTS OUTPUT
    ...
    with open(file_name, 'w') as f:
        ...
        print("\n\n### OUTPUTS ###\n", file=f)
        print("BC - RC correlation:", file=f)
        print(scipy.stats.pearsonr(nodes_B, nodes_R), file=f)
